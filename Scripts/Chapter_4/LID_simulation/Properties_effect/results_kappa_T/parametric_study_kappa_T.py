...
for alpha in alphas:
    for i in range(3):
        prms.append([E_maxs[i], durations[i], E_dt, eta_tr, phi, alpha])

mat = "W"
output_folder = ("../results_kappa_T",)
for i in range(1, 26):
    print("\n")
    print(f"Iteration: {i}")

    E_max, dur, E_dt, eta_tr, phi, alpha = prms[N - 1]

    E = E_max * i / 25 * np.sqrt(alpha)
    ...
    filename = f"../results_kappa_T/fluxes_{mat}_{dur}_E{E/1e6:.5f}MJ_Edt{E_dt:.5f}eV_eta{-np.log10(eta_tr):.5f}_phi{-np.log10(phi):.5f}_alpha{alpha:.2f}.csv"
