...
for alpha in alphas:
    for phi in phis:
        for i in range(3):
            prms.append([E_maxs[i], durations[i], alpha, phi])

mat = "W"
output_folder = "../results_kappa_phi/"

#print("\n")
#print(f"Iteration: {i}")

E_max, dur, alpha, phi = prms[N-1] 
E = E_max
filename = f"{mat}_{dur}_E{E/1e6:.5f}MJ_Edt{E_dt:.5f}eV_eta{-np.log10(eta_tr):.5f}_phi{-np.log10(phi):.5f}_alpha{alpha:.2f}"

print(filename)

flux_out = f"{output_folder}/fluxes_{filename}.csv"
...
np.savetxt(flux_out, out, delimiter=",") 