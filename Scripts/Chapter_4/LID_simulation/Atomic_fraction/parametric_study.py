...
for eta_tr in eta_trs:
    for phi in phis:
        for i in range(3):
            prms.append([E_maxs[i], durations[i], E_dt, eta_tr, phi])
...
for i in range(1, 26):
    ...
    E_max, dur, E_dt, eta_tr, phi = prms[N-1] 
    ...
    header = "t(s),Jat(m^-2s^-1),Jmol_s(m^-2s^-1),Jmol_sb(m^-2s^-1),Jmol_b(m^-2s^-1),Jdes(m^-2s^-1)"
    filename = f"../results/fluxes_{mat}_{dur}_E{E/1e6:.5f}MJ_Edt{E_dt:.5f}eV_eta{-np.log10(eta_tr):.5f}_phi{-np.log10(phi):.5f}.csv"
    np.savetxt(filename, out, header=header, delimiter=",")