...
Elim=$(awk -v ArrayTaskID=$SLURM_ARRAY_TASK_ID '$1==ArrayTaskID {print $7}' $config)

filename=ELM_${SLURM_ARRAY_JOB_ID}_${q_stat}MW_${f_ELM}Hz_${Elim}eV
dir=$PWD
mkdir ${filename}
...
my_model.sources = [stat_source, elm_source]

my_model.initial_conditions =[
     F.InitialCondition(field="0", value=0.),
     F.InitialCondition(field="1", value=0.)
]
...
my_model.boundary_conditions = [
    ...
    F.CustomFlux(function=recombination_Og, field="solute", surfaces=1),
    F.DirichletBC(value=0, field=0, surfaces=2)
]
...
class CustomStepsize(F.Stepsize):
    def adapt(self, t, nb_it, converged):  # override the adapt method
        ...
        if (t + 5 * float(self.value)) % CYCLE_TIME < ELM_TIME:
...
class CustomHydrogenFlux(F.SurfaceQuantity):
    def __init__(self, surface) -> None:
        super().__init__(field="solute", surface=surface)
        self.title = "Flux surface {}: {}".format(self.surface, self.field)

    def compute(self, soret=True):
        flux = f.assemble(
            self.D * f.dot(f.grad(self.function), self.n) * self.ds(self.surface)
        )
        if soret and self.field in [0, "0", "solute"]:
            if self.heat_transport is callable:
                self.heat_transport = self.heat_transport(self.T)
            T = self.T
            flux += f.assemble(
                self.D
                * self.function
                * self.heat_transport
                / (F.R * T**2)
                * f.dot(f.grad(T), self.n)
                * self.ds(self.surface)
            )
        return flux

class RecyclingCoefficient(CustomHydrogenFlux):
    def __init__(self, surface) -> None:
...
derived_quantities = [CustomDerivedQuantities([CustomHydrogenFlux(surface=1), 
