...
'''vertices = np.concatenate((
    np.linspace(0, 1e-7, num = 100),
    np.linspace(1e-7, 1e-3, num = 20000),
    np.linspace(1e-3, 6e-3, num = 10000),
))'''
...
my_model.dt = F.Stepsize(
    ...
    max_stepsize = lambda t: None if t < 5 else 5,
    dt_min=1e-8
)
...
my_model.exports = XDMF +  derived_quantities 
