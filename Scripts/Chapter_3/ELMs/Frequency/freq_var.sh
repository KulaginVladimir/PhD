...
filename=ELM_${SLURM_ARRAY_JOB_ID}_${q_stat}MW_${f_ELM}Hz_${time}s
dir=$PWD
mkdir ${filename}
...
my_model.dt = F.Stepsize(
    initial_value=in_ts,
    stepsize_change_ratio=1.05,
    max_stepsize = lambda t: None if t < 1e-4 else 5e-5,
    dt_min=1e-8
)
...
derived_quantities = [F.DerivedQuantities([F.TotalSurface(field='T', surface=1), 
