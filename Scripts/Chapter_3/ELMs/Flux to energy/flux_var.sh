...
flux=$(awk -v ArrayTaskID=$SLURM_ARRAY_TASK_ID '$1==ArrayTaskID {print $7}' $config)

filename=ELM_${SLURM_ARRAY_JOB_ID}_${q_stat}MW_${f_ELM}Hz_${time}s_${flux}
...
ELM_params = {
    940:    [5.29e-1, 3.22e-1, 1.44e-8, 7.83e-9],
    1880:   [4.74e-1, 2.73e-1, 2.27e-8, 1.23e-8],
    3760:   [4.07e-1, 2.18e-1, 3.69e-8, 1.98e-8],
...
def q_tot(t):
    t = t % (1 / f_ELM)
    return q_stat + q_elm_max*(1+(tau/t)**2)*(tau/t)**2*sp.exp(-(tau/t)**2)
def rad(T, mobile):
...
def cooling(T, mobile):
    if q_stat == 10e6:
        ...
        params = {# "freq": q_eqv * (a + b*x+ c*exp(-d*x))
            0:   [1.388e-1, 1.287e-2, 1.334e-1, np.log(7.922e-1)],
            10:  [1.305e-1, 6.830e-3, 1.165e-1, np.log(8.852e-1)],
            20:  [1.281e-1, 5.770e-3, 1.170e-1, np.log(8.949e-1)],
            30:  [1.264e-1, 5.160e-3, 1.175e-1, np.log(9.033e-1)],
            40:  [1.252e-1, 4.740e-3, 1.182e-1, np.log(9.097e-1)],
            50:  [1.242e-1, 4.430e-3, 1.188e-1, np.log(9.147e-1)],
            60:  [1.234e-1, 4.180e-3, 1.197e-1, np.log(9.188e-1)],
            70:  [1.227e-1, 3.970e-3, 1.206e-1, np.log(9.223e-1)],
            80:  [1.221e-1, 3.800e-3, 1.215e-1, np.log(9.253e-1)],
            90:  [1.215e-1, 3.650e-3, 1.196e-1, np.log(9.278e-1)],
            100: [1.211e-1, 3.520e-3, 1.336e-1, np.log(9.300e-1)]}
    elif q_stat == 1e6:
        params = {# "freq": q_eqv * (a + b*x+ c*exp(-d*x))
    ...
    return -(q_stat + eps_trans*f_ELM) * (params[f_ELM][0] + params[f_ELM][1] * (T-T0) - params[f_ELM][2] * f.exp(params[f_ELM][3]*(T-T0)))
    
def flux_elm(t):
    t = t % (1 / f_ELM)
    return q_elm_max*(tau/t)**2*sp.exp(-(tau/t)**2) / T_eped / (1-r_en_ELM) / q
...
elm_source = F.ImplantationFlux(
    flux=ff*flux_elm(F.t)*(1-r_ELM)*norm_flux(X_ELM, sigma_ELM),  # H/m2/s
    imp_depth=X_ELM,  # m
    width=sigma_ELM,  # m
    volume=1
)

my_model.sources = [stat_source, elm_source]#, rec_source]
...
class CustomStepsize(F.Stepsize):
    def adapt(self, t, nb_it, converged):  # override the adapt method
        super().adapt(t, nb_it, converged)  # call the parent method

        # cap the stepsize during the ELM
        if (t + 3.5 * float(self.value)) % CYCLE_TIME < ELM_TIME:
            if float(self.value) > max_stepsize_during_elm:
                self.value.assign(max_stepsize_during_elm)


my_model.dt = CustomStepsize(
    initial_value=1e-8,
    stepsize_change_ratio=1.05,
    t_stop = 0.0,
    stepsize_stop_max = 5e-3,
    dt_min=1e-8
)
...
derived_quantities = [F.DerivedQuantities([F.HydrogenFlux(surface=1), 
                                          ...
                                          nb_iterations_between_compute = 5,
