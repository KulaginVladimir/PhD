...
filename=EQV_${SLURM_ARRAY_JOB_ID}_${q_stat}MW_${f_ELM}Hz_${time}s_${Edt}eV_${eta_tr}
dir=$PWD
mkdir ${filename}
...
q_elm_max=eps_trans/(tau**2*f_ELM/2*sp.exp(-(tau*f_ELM)**2)+3/4*sp.sqrt(np.pi)*tau*special.erfc(f_ELM*tau))
...
q_eqv = eps_trans * f_ELM
Gamma_eqv = q_elm_max / T_eped / (1-r_en_ELM) / q * (1 / 2 * np.sqrt(np.pi) * tau * f_ELM * special.erfc(f_ELM*tau))
...
def q_tot(T, mobile):
    return q_stat + q_eqv - alpha_W * sigma * (T ** 4 - T0 ** 4)
...
def cooling(T, mobile):
    ...
    return -(q_stat + q_eqv) * (params[f_ELM][0] + params[f_ELM][1] * (T-T0) - params[f_ELM][2] * f.exp(params[f_ELM][3]*(T-T0)))
...
my_model.mesh = F.MeshFromVertices(vertices=np.array(mesh))
...
elm_source = F.ImplantationFlux(
    flux=Gamma_eqv*(1-r_ELM)*norm_flux(X_ELM, sigma_ELM),  # H/m2/s
    imp_depth=X_ELM,  # m
    width=sigma_ELM,  # m
    volume=1
)
...
my_model.boundary_conditions = [
    ...
    F.DirichletBC(surfaces=[1,2], value=0.0, field=0),
]
...
my_model.T = F.HeatTransferProblem(
    ...
    initial_condition=F.InitialCondition(value=T0),
